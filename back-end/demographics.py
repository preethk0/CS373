...
def filter_demographics(dem_query, queries):
    ...
    if "country_population" in queries:
        ...
        for filter in population_filter:
            ...
            all_filters.append(
                and_(
                    Demographics.country_population > int(lower_bound) * 10 ** 6,
                    Demographics.country_population <= int(upper_bound) * 10 ** 6,
                )
            )
    ...
    if "country_gdp" in queries:
        ...
        for filter in gdp_filter:
            ...
            all_filters.append(
                and_(
                    Demographics.country_GDP > int(lower_bound) * 10 ** 9,
                    Demographics.country_GDP <= int(upper_bound) * 10 ** 9,
                )
            )
        dem_query = dem_query.filter(or_(*tuple(all_filters)))
    ...
    if "country_states" in queries:
        ...
        for filter in states_filter:
            ...
            all_filters.append(
                and_(
                    Demographics.country_states >= int(lower_bound),
                    Demographics.country_states <= int(upper_bound),
                )
            )
        dem_query = dem_query.filter(or_(*tuple(all_filters)))
...
def sort_demographics(dem_query, queries):
    if "sort" in queries:
        ...
        if dem_attribute:
            return dem_query.order_by(
                dem_attribute.desc() if order == "des" else dem_attribute
            )
...
def search_demographics(dem_query, queries):
    if "search" in queries:
        ...
        if term.find(keywords[0]) == 0:
            all_filters.append(
                func.lower(
                    cast(Demographics.country_population, sqlalchemy.String)
                ).startswith(term[len(keywords[0]) + 1 :])
            )  # Check populations
        elif term.find(keywords[1]) == 0:
            all_filters.append(
                func.lower(Demographics.country_languages).startswith(
                    term[len(keywords[1]) + 1 :]
                )
            )  # Check languages
        elif term.find(keywords[2]) == 0:
            all_filters.append(
                func.lower(
                    cast(Demographics.country_states, sqlalchemy.String)
                ).startswith(term[len(keywords[2]) + 1 :])
            )  # Check number of states
        elif term.find(keywords[3]) == 0:
            all_filters.append(
                func.lower(
                    cast(Demographics.country_GDP, sqlalchemy.String)
                ).startswith(term[len(keywords[3]) :])
            )  # Check nominal GDP

        all_filters.append(
            func.lower(Demographics.country_name).contains(term)
        )  # Check country names
        all_filters.append(
            func.lower(Demographics.country_capital).contains(term)
        )  # Check capitals
        all_filters.append(
            func.lower(
                cast(Demographics.country_population, sqlalchemy.String)
        ...
        all_filters.append(
            func.lower(Demographics.country_languages).contains(term)
        )  # Check languages
        all_filters.append(
            func.lower(cast(Demographics.country_states, sqlalchemy.String)).contains(
                term
            )
        )  # Check number of states
        all_filters.append(
            func.lower(cast(Demographics.country_GDP, sqlalchemy.String)).contains(term)
        )  # Check GDP
