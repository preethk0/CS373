...
def filter_geography(geo_query, queries):
    ...
    if "country_longitude" in queries:
        ...
        for filter in longitude_filter:
            ...
            all_filters.append(
                and_(
                    Geography.country_longitude > int(lower_bound),
                    Geography.country_longitude <= int(upper_bound),
                )
            )
    ...
    if "country_latitude" in queries:
        ...
        for filter in latitude_filter:
            ...
            all_filters.append(
                and_(
                    Geography.country_latitude > int(lower_bound),
                    Geography.country_latitude <= int(upper_bound),
                )
            )
        geo_query = geo_query.filter(or_(*tuple(all_filters)))
...
def sort_geography(geo_query, queries):
    if "sort" in queries:
        ...
        if geo_attribute:
            return geo_query.order_by(
                geo_attribute.desc() if order == "des" else geo_attribute
            )
...
def search_geography(geo_query, queries):
    if "search" in queries:
        ...
        all_filters.append(
            func.lower(cast(Geography.country_longitude, sqlalchemy.String)).contains(
                term
            )
        )
        all_filters.append(
            func.lower(cast(Geography.country_latitude, sqlalchemy.String)).contains(
                term
            )
        )
